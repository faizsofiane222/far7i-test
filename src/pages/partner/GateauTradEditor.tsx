...
import {
    ...
    Loader2, Info, Sparkles, DollarSign, Truck, Box, Palette,
} from "lucide-react";
...
const PRODUIT_TYPES = [
    { key: "individuels", label: "Gâteaux traditionnels individuels", emoji: "🍮" },
    { key: "assortis", label: "Gâteaux traditionnels assortis (par lot)", emoji: "🫙" },
];

const CARACTERISTIQUES = [
    { key: "vente_piece", label: "Vente à la pièce", icon: Box },
    { key: "lots", label: "Possibilité de lots", icon: Box },
    { key: "boites_presentation", label: "Mise en forme dans des boîtes + Présentation personnalisée", icon: Sparkles },
];
...
export default function GateauTradEditor({ providerIdProp }: { providerIdProp?: string }) {
    const { id } = useParams();
    const { user } = useAuth();
        ...
        try {
            ...
            if (p) {
                ...
                if (catering) {
                    ...
                    CARACTERISTIQUES.forEach(c => { if (opts.includes(c.key)) cMap[c.key] = true; });
                    setCaracteristiques(cMap);
    ...
    const toggleCarac = (key: string) =>
        setCaracteristiques(prev => ({ ...prev, [key]: !prev[key] }));
    const toggleWilaya = (code: string) =>
        setWilayasLivraison(prev => prev.includes(code) ? prev.filter(c => c !== code) : [...prev, code]);
    ...
    const handleSave = async (isDraft: boolean = false) => {
        ...
        try {
            ...
            const providerPayload: any = {
                ...
                category_slug: "gateau_traditionnel",
                wilaya_id: wilayaId || null, address,
            ...
            const deliveryOptions = [
                ...Object.entries(caracteristiques).filter(([, v]) => v).map(([k]) => k),
        ...
        <div className="max-w-4xl mx-auto pb-36 mt-8 px-4">
            <div className="mb-12 space-y-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-serif font-bold text-[#1E1E1E]">Gâteau Traditionnel & Moderne</h1>
            ...
            <div className="bg-white rounded-3xl border border-[#D4D2CF] p-8 md:p-12 shadow-sm min-h-[500px] relative overflow-hidden">
                <div className="absolute top-0 right-0 p-8 opacity-[0.04]"><span className="text-[8rem]">🍮</span></div>
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                ...
                                <GildedInput value={commercialName} onChange={e => setCommercialName(e.target.value)} placeholder="Ex: Pâtisserie Baya, Artisan Amouri..." />
                            </div>
                            <div className="space-y-2">
                ...
                {step === 2 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <UXHint>Présentez uniquement vos créations réelles. Une sélection courte rassure davantage le client.</UXHint>
                    </div>
                )}
                ...
                {step === 3 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <textarea value={description} onChange={e => setDescription(e.target.value)}
                            ...
                            placeholder="Tradition, spécialité, savoir-faire, ce qui vous différencie..." />
                        <UXHint>Ce champ est facultatif. Il permet au client de comprendre votre style.</UXHint>
                    </div>
                )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {PRODUIT_TYPES.map(type => (
                                <button key={type.key} onClick={() => toggleProduit(type.key)}
                                    ...
                                    <span className="font-bold uppercase text-sm tracking-widest text-center">{type.label}</span>
                        ...
                        <div>
                            ...
                            <div className="space-y-3">
                                {CARACTERISTIQUES.map(c => (
                                    <ToggleCard key={c.key} label={c.label} active={!!caracteristiques[c.key]} onToggle={() => toggleCarac(c.key)} icon={<c.icon className="w-5 h-5" />} />
                                ))}
                            </div>
                        </div>
                        <div>
                            <p className="text-xs font-bold uppercase tracking-widest text-[#1E1E1E]/40 mb-3">Personnalisation</p>
                            <ToggleCard
                                label="Personnalisation possible (forme, finition, décoration — ex: noir et blanc)"
                                active={personnalisationPossible}
                                onToggle={() => setPersonnalisationPossible(!personnalisationPossible)}
                                icon={<Palette className="w-5 h-5" />}
                            />
                        </div>
                        <UXHint>Ces éléments aident le client à comprendre comment sont proposés vos produits et ce qui peut être adapté.</UXHint>
                    </div>
                )}
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea value={politiqueAnnulation} onChange={e => setPolitiqueAnnulation(e.target.value)}
                                ...
                                placeholder="Ex: Annulation gratuite jusqu'à 5 jours avant la commande..." />
                        </div>
                    </div>
                )}
