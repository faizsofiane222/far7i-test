...
export default function LogisticsStep() {
    ...
    const toggleWilaya = (code: string) => {
        if (wilayasDeplacement.includes(code)) {
            setValue("wilayasDeplacement", wilayasDeplacement.filter((w: string) => w !== code), { shouldValidate: true });
        } else {
            setValue("wilayasDeplacement", [...wilayasDeplacement, code], { shouldValidate: true });
        }
    };
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 font-lato">
            <div className="mb-6">
                ...
                <p className="text-sm text-[#1E1E1E]/80 mb-6 font-lato">Définissez vos zones d'intervention et les régions que vous couvrez.</p>
            </div>
            ...
            <div className="space-y-6 bg-[#EBE6DA] p-6 rounded-2xl border border-[#D4D2CF]/50">
                <div className="flex items-center justify-between p-4 bg-white border border-[#D4D2CF] rounded-xl hover:border-[#B79A63] transition-colors cursor-pointer"
                    ...
                    <div>
                        ...
                        <span className="text-xs text-[#1E1E1E]/70 font-lato mt-1 block">La troupe accepte-t-elle de se déplacer hors de sa wilaya ?</span>
                    </div>
