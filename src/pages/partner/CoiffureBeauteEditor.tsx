...
import {
    ...
    Loader2, Info, Sparkles, DollarSign, MapPin, Scissors,
} from "lucide-react";
...
const PRESTATIONS = [
    ...
    { key: "maquillage", label: "Maquillage", emoji: "💄" },
];
...
export default function CoiffureBeauteEditor({ providerIdProp }: { providerIdProp?: string }) {
    const { id } = useParams();
    const { user } = useAuth();
    ...
    const [retouchesPendant, setRetouchesPendant] = useState(false);
    const [prestationSalon, setPrestationSalon] = useState(false);
    const [prestationLieu, setPrestationLieu] = useState(false);
        ...
        try {
            ...
            if (p) {
                ...
                if (beauty) {
                    ...
                    setRetouchesPendant(allOpts.includes("retouches_pendant"));
                    setPrestationSalon(allOpts.includes("prestation_salon"));
                    setPrestationLieu(allOpts.includes("prestation_lieu"));
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const providerPayload: any = {
                commercial_name: commercialName, category_slug: "coiffure_beaute",
            ...
            const allOptions = [
                ...(retouchesPendant ? ["retouches_pendant"] : []),
                ...(prestationSalon ? ["prestation_salon"] : []),
                ...(prestationLieu ? ["prestation_lieu"] : []),
        ...
        <div className="max-w-4xl mx-auto pb-36 mt-8 px-4">
            <div className="mb-12 space-y-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-serif font-bold text-[#1E1E1E]">Coiffure & Beauté</h1>
            ...
            <div className="bg-white rounded-3xl border border-[#D4D2CF] p-8 md:p-12 shadow-sm min-h-[500px] relative overflow-hidden">
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <Label className="text-sm font-bold text-[#1E1E1E]">Nom de la coiffeuse / maquilleuse / salon *</Label>
                                <GildedInput value={commercialName} onChange={e => setCommercialName(e.target.value)} placeholder="Ex: Salon Nour, Makeup by Amina..." />
                            </div>
                            <div className="space-y-2">
                        ...
                        <div className="space-y-4">
                            ...
                            <UXHint>Sélectionnez uniquement les événements pour lesquels vous intervenez réellement. Votre prestation sera visible uniquement pour ces événements.</UXHint>
                        </div>
                    </div>
                )}
                ...
                {step === 2 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <UXHint>Présentez vos réalisations réelles. Une sélection courte et qualitative inspire plus de confiance.</UXHint>
                    </div>
                )}
                ...
                {step === 3 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <textarea value={description} onChange={e => setDescription(e.target.value)}
                            ...
                            placeholder="Style, spécialité, expérience..." />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {PRESTATIONS.map(p => (
                                <button key={p.key} onClick={() => togglePrestation(p.key)}
                                    className={cn("flex flex-col items-center gap-4 p-12 rounded-2xl border-2 transition-all",
                                        prestations[p.key] ? "bg-[#1E1E1E] border-[#1E1E1E] text-white" : "bg-white border-[#D4D2CF] text-[#1E1E1E]/60")}>
                                    <span className="text-6xl">{p.emoji}</span>
                                    <span className="font-bold uppercase text-sm tracking-widest">{p.label}</span>
                ...
                {step === 5 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <ToggleCard label="Retouches pendant l'événement" active={retouchesPendant} onToggle={() => setRetouchesPendant(!retouchesPendant)} icon={<Scissors className="w-5 h-5" />} />
                ...
                {step === 6 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="space-y-3">
                            <ToggleCard label="Prestation en salon" active={prestationSalon} onToggle={() => setPrestationSalon(!prestationSalon)} icon={<Sparkles className="w-5 h-5" />} />
                            <ToggleCard label="Prestation sur le lieu de l'événement" active={prestationLieu} onToggle={() => {
                                setPrestationLieu(!prestationLieu);
                                if (!prestationLieu) setDeplacementPossible(true);
                                else { setDeplacementPossible(false); setWilayasDeplacement([]); }
                        ...
                        <UXHint>Les modalités sont définies lors de l'échange avec le client.</UXHint>
                    </div>
                )}
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="space-y-2">
                            ...
                            <textarea value={politiqueAnnulation} onChange={e => setPolitiqueAnnulation(e.target.value)}
                                ...
                                placeholder="Ex: Annulation gratuite jusqu'à 48h avant la date..." />
                        </div>
                    </div>
                )}
