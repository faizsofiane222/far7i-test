...
import {
    ...
    Loader2, Info, Sparkles, DollarSign, Car, Clock,
    Shield, Flower2,
} from "lucide-react";
...
const PRESTATIONS = [
    { key: "avec_chauffeur", label: "Location avec chauffeur", emoji: "🚘" },
    { key: "sans_chauffeur", label: "Location sans chauffeur", emoji: "🔑" },
    { key: "vintage", label: "Véhicules vintage", emoji: "🚗" },
    { key: "utilitaire", label: "Véhicules utilitaires (master)", emoji: "🚐" },
];

const OPTIONS = [
    { key: "decoration", label: "Décoration du véhicule", icon: Flower2 },
    { key: "presence_prolongee", label: "Présence prolongée", icon: Clock },
    { key: "vehicule_remplacement", label: "Véhicule de remplacement (si nécessaire)", icon: Car },
];

const ORGANISATION = [
    { key: "a_lheure", label: "Mise à disposition à l'heure" },
    { key: "demi_journee", label: "Mise à disposition à la demi-journée" },
    { key: "journee", label: "Mise à disposition à la journée" },
    { key: "prise_en_charge", label: "Prise en charge et dépose au lieu choisi" },
];
...
const STEPS = [
    ...
    { num: 5, label: "Options" },
    { num: 6, label: "Organisation" },
    { num: 7, label: "Tarifs" },
    { num: 8, label: "Conditions" },
];

export default function LocationVoitureEditor({ providerIdProp }: { providerIdProp?: string }) {
    const { id } = useParams();
    const { user } = useAuth();
    ...
    const [organisation, setOrganisation] = useState<Record<string, boolean>>({});
    const [basePrice, setBasePrice] = useState<number | "">("");
    const [acompteDemande, setAcompteDemande] = useState(false);
    const [cautionDemandee, setCautionDemandee] = useState(false);
    ...
    const fetchProviderData = async () => {
        ...
        try {
            const { data: p } = await supabase.from("providers")
                .select(`id, commercial_name, wilaya_id, address, events_accepted, bio, base_price,
            ...
            if (p) {
                ...
                if (beauty) {
                    ...
                    setPrestations(pMap);
                    const allOpts: string[] = beauty.options || [];
                    ...
                    ORGANISATION.forEach(o => { if (allOpts.includes(o.key)) orgMap[o.key] = true; });
                    setOrganisation(orgMap);
                    setAcompteDemande(allOpts.includes("acompte"));
                    setCautionDemandee(allOpts.includes("caution"));
    ...
    const toggleOrganisation = (key: string) =>
        setOrganisation(prev => ({ ...prev, [key]: !prev[key] }));
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const providerPayload: any = {
                commercial_name: commercialName, category_slug: "location_voiture",
            ...
            const allOptions = [
                ...
                ...Object.entries(organisation).filter(([, v]) => v).map(([k]) => k),
                ...(acompteDemande ? ["acompte"] : []),
                ...(cautionDemandee ? ["caution"] : []),
        ...
        <div className="max-w-4xl mx-auto pb-36 mt-8 px-4">
            ...
            <div className="mb-12 space-y-8">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-3xl font-serif font-bold text-[#1E1E1E]">Location de Voiture</h1>
            ...
            <div className="bg-white rounded-3xl border border-[#D4D2CF] p-8 md:p-12 shadow-sm min-h-[500px] relative overflow-hidden">
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <Label className="text-sm font-bold text-[#1E1E1E]">Nom de l'agence / prestataire *</Label>
                                <GildedInput value={commercialName} onChange={e => setCommercialName(e.target.value)} placeholder="Ex: Auto Prestige, Location Saouli..." />
                            </div>
                            <div className="space-y-2">
                        ...
                        <div className="space-y-4">
                            ...
                            <UXHint>Sélectionnez uniquement les événements pour lesquels vous proposez la location de voiture. Votre prestation sera visible uniquement pour ces événements.</UXHint>
                        </div>
                    </div>
                )}
                ...
                {step === 2 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <UXHint>Présentez des véhicules réellement disponibles. Une sélection courte et qualitative inspire plus de confiance.</UXHint>
                    </div>
                )}
                ...
                {step === 3 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <textarea value={description} onChange={e => setDescription(e.target.value)}
                            ...
                            placeholder="Type de véhicules, standing, expérience de l'agence..." />
                        <UXHint>Ce champ est facultatif. Il aide le client à comprendre votre offre.</UXHint>
                    </div>
                )}
                ...
                {step === 4 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            {PRESTATIONS.map(p => (
                                <button key={p.key} onClick={() => togglePrestation(p.key)}
                                    className={cn("flex flex-col items-center gap-4 p-8 rounded-2xl border-2 transition-all",
                                        prestations[p.key] ? "bg-[#1E1E1E] border-[#1E1E1E] text-white" : "bg-white border-[#D4D2CF] text-[#1E1E1E]/60")}>
                                    <span className="text-4xl">{p.emoji}</span>
                                    <span className="font-bold uppercase text-xs tracking-widest text-center">{p.label}</span>
                                    {prestations[p.key] && <Check className="w-5 h-5 text-[#B79A63]" />}
                                </button>
                            ))}
                        </div>
                ...
                {step === 6 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        <h2 className="text-2xl font-serif text-[#1E1E1E]">Organisation <span className="text-[#B79A63]">(informatif)</span></h2>
                        <div className="space-y-3">
                            {ORGANISATION.map(o => (
                                <ToggleCard key={o.key} label={o.label} active={!!organisation[o.key]} onToggle={() => toggleOrganisation(o.key)} icon={<Car className="w-5 h-5" />} />
                            ))}
                        </div>
                        <UXHint>Les modalités exactes sont définies lors de l'échange avec le client.</UXHint>
                    </div>
                )}
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="bg-[#F8F5F0] p-8 rounded-3xl border border-[#D4D2CF]/50 flex flex-col items-center justify-center text-center gap-4">
                                ...
                                <p className="text-xs text-[#1E1E1E]/60 leading-relaxed">Le tarif final dépend de l'organisation réelle.</p>
                            </div>
                        </div>
                ...
                {step === 8 && (
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2">
                        ...
                        <div className="space-y-3">
                            ...
                            <ToggleCard label="Caution demandée (si sans chauffeur)" active={cautionDemandee} onToggle={() => setCautionDemandee(!cautionDemandee)} icon={<Shield className="w-5 h-5" />} />
                        </div>
                        <div className="space-y-2">
                            ...
                            <textarea value={politiqueAnnulation} onChange={e => setPolitiqueAnnulation(e.target.value)}
                                ...
                                placeholder="Ex: Annulation gratuite jusqu'à 72h avant la date..." />
                        </div>
                    </div>
                )}
