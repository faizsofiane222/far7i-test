...
export default function MediaStep() {
    ...
    const utiliserFormulaireFar7i = useWatch({ name: "utiliserFormulaireFar7i" });
    const photos = useWatch({ name: "galeriePhotos" }) || [];

    // Mock functions for Freemium 5 photo gallery
    const handleMockUpload = () => {
        if (photos.length >= 5) return;
        const mockUrl = `https://picsum.photos/800/600?random=${Math.random()}`;
        setValue("galeriePhotos", [...photos, mockUrl], { shouldValidate: true });
    };
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 font-lato">
            <div className="mb-6">
                ...
                <p className="text-sm text-[#1E1E1E]/80 mb-6 font-lato">Mettez en avant vos plus belles prestations (5 photos max).</p>
            </div>
                ...
                <div>
                    <div className="flex justify-between items-center mb-4">
                        ...
                        {photos.length < 5 && (
                            ...
                            >
                                <Plus className="w-3 h-3" /> Ajouter (Mock)
                            </button>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        ...
                        {photos.length === 0 && (
                            <div className="col-span-full h-32 border-2 border-dashed border-[#B79A63]/30 rounded-xl flex flex-col items-center justify-center bg-[#F8F5F0]">
                                ...
                                <span className="text-[#1E1E1E]/40 text-xs mt-1">Cliquez sur Ajouter (Mock)</span>
                            </div>
                        )}
                    </div>
                    {errors.galeriePhotos && <p className="text-red-500 text-xs mt-2">{errors.galeriePhotos.message as string}</p>}
                </div>

                <div className="pt-4 border-t border-[#D4D2CF]/50 space-y-6">
                    <h3 className="text-lg font-serif font-bold text-[#1E1E1E]">Contact Client</h3>
                    ...
                    <div className="flex items-center justify-between p-4 bg-white border border-[#D4D2CF] rounded-xl hover:border-[#B79A63] transition-colors cursor-pointer"
                        onClick={() => setValue("utiliserFormulaireFar7i", !utiliserFormulaireFar7i, { shouldValidate: true })}>
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <span className="block font-bold text-sm text-[#1E1E1E]">Recevoir des demandes via Far7i</span>
                                <span className="text-xs text-[#1E1E1E]/70 font-lato mt-1 block">Laisser les clients vous contacter par formulaire</span>
                            </div>
                        </div>
                        <div className={cn("w-12 h-6 rounded-full transition-colors relative flex-shrink-0", utiliserFormulaireFar7i ? "bg-[#B79A63]" : "bg-[#D4D2CF]/80")}>
                            <div className={cn("absolute top-1 w-4 h-4 rounded-full bg-white transition-all", utiliserFormulaireFar7i ? "left-7" : "left-1")} />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div className="relative">
                            <input
                                ...
                                className={cn("w-full h-12 pl-12 pr-4 rounded-xl border bg-white text-[#1E1E1E] focus:outline-none focus:border-[#B79A63] transition-colors", errors.telephone ? "border-red-500" : "border-[#D4D2CF]")}
                            />
                            <Phone className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[#B79A63]" />
                        </div>
                        <p className="text-xs text-[#1E1E1E]/80 mt-2">Un numéro que les clients peuvent joindre directement.</p>
