...
import { beauteSchema, type BeauteFormValues } from "./coiffure-beaute/schema";
import IdentityStep from "./coiffure-beaute/steps/IdentityStep";
import SpecialtiesStep from "./coiffure-beaute/steps/SpecialtiesStep";
import LogisticsStep from "./coiffure-beaute/steps/LogisticsStep";
import PricingStep from "./coiffure-beaute/steps/PricingStep";
import MediaStep from "./coiffure-beaute/steps/MediaStep";
...
export default function BeauteWizard() {
    const { id } = useParams();
    const { user } = useAuth();
    ...
    const methods = useForm<BeauteFormValues>({
        ...
        defaultValues: {
            ...
            retouchesPendantEvenement: false,
            organisation: {
                prestationSalon: false,
                prestationSurLieu: false,
            },
    ...
    const fetchProviderData = async () => {
        try {
            ...
            if (provider) {
                ...
                const hasCoiffure = featuresList.includes("specialite:coiffure");
                const hasMaquillage = featuresList.includes("specialite:maquillage");

                const retouchesPendantEvenement = featuresList.includes("option:retouches_evenement");

                const prestationSalon = featuresList.includes("lieu:salon");
                const prestationSurLieu = featuresList.includes("lieu:externe");
    ...
    const onSubmit = async (data: BeauteFormValues, isDraftOrEvent?: boolean | React.BaseSyntheticEvent) => {
        ...
        try {
            ...
            const propertyFeatures = [
                ...(data.specialites.hasCoiffure ? ["specialite:coiffure"] : []),
                ...(data.specialites.hasMaquillage ? ["specialite:maquillage"] : []),
                ...(data.retouchesPendantEvenement ? ["option:retouches_evenement"] : []),
                ...(data.organisation.prestationSalon ? ["lieu:salon"] : []),
                ...(data.organisation.prestationSurLieu ? ["lieu:externe"] : []),
            ...
            const detailsPayload = {
                ...
                service_options: [], // We use property_features for strict toggles
    ...
    return (
        <div className="min-h-screen bg-[#FDFCFB] pb-24 font-lato">
            <div className="max-w-4xl mx-auto px-4 pt-12">
                {/* Stepper Header */}
                <div className="mb-12">
                    <h1 className="text-4xl font-serif text-[#1E1E1E] mb-8 text-center">Coiffure & Beauté</h1>
